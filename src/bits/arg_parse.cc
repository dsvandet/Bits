...
const char *bits::require_find_argument(const char *name, int argc, const char **argv) {
...
const char *bits::find_argument(const char *name, int argc, const char **argv) {
...
void bits::check_for_unknown_arguments(
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!matched) {
            ...
            if (for_mode == nullptr) {
                ...
                msg << "Unrecognized command line argument " << argv[i] << " for `bits " << for_mode << "`.\n";
                msg << "Recognized command line arguments for `bits " << for_mode << "`:\n";
            }
...
bool bits::find_bool_argument(const char *name, int argc, const char **argv) {
...
int64_t bits::find_int64_argument(
...
float bits::find_float_argument(
...
FILE *bits::find_open_file_argument(
...
ostream_else_cout bits::find_output_stream_argument(
...
std::vector<std::string> bits::split(char splitter, const std::string &text) {
...
double bits::parse_exact_double_from_string(const std::string &text) {
    char *end = nullptr;
...
uint64_t bits::parse_exact_uint64_t_from_string(const std::string &text) {
    char *end = nullptr;
    const char *c = text.c_str();
