...
#ifndef _BITS_MEM_POINTER_RANGE_H
#define _BITS_MEM_POINTER_RANGE_H
...
template <typename T>
std::ostream &operator<<(std::ostream &out, const bits::ConstPointerRange<T> &v) {
    out << "ConstPointerRange{" << comma_sep(v) << "}";
    return out;
}

template <typename T>
std::ostream &operator<<(std::ostream &out, const bits::PointerRange<T> &v) {
