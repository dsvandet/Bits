...
std::vector<size_t> bits::sample_hit_indices(float probability, size_t attempts, std::mt19937_64 &rng) {
...
std::mt19937_64 bits::externally_seeded_rng() {
    ...
    // See https://github.com/quantumlib/bits/issues/26
...
std::mt19937_64 bits::optionally_seeded_rng(int argc, const char **argv) {
...
void bits::biased_randomize_bits(float probability, uint64_t *start, uint64_t *end, std::mt19937_64 &rng) {
    if (probability > 0.5) {
